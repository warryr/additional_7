module.exports = function solveSudoku(matrix) {
    ...
    solvation(matrix, temp, i = 0, j = 0);
    return matrix;
    
    function solvation(A, B, i, j){
        if (isGivenCondition(A, i, j)){
            if (isLast(A, i, j)){
                return true;
            } else{
                if (j == A.length - 1){
                    return solvation(A, B, i + 1, 0);
                } else {
                    return solvation(A, B, i, j + 1);
                }
            }
        } else{
            let values = B[i][j];
            for (let num in values){
                ...
                if (!hasBroken(A, i, j)){
                    ...
                    let result;
                    if (j == A.length - 1){
                        result = solvation(A, B, i + 1, 0);
                    } else{
                        result = solvation(A, B, i, j + 1);
                    }
                    if (result){
                        return true;
                    }
